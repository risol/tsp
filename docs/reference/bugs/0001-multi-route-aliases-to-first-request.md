...
Message::Execute { id, request: ExecuteRequest { method, path, script, ... } }
  -> tsp_worker.rs::execute_request
     -> writes request.script to the worker-process temp file
        `tsp-embedded-worker-<pid>.tsx`
...
worker is a separate `tspserver.exe` process with its own
...
- `bun/src/bundler/entry_points.rs` — `ServerEntryPoint::generate`
  ...
  them.
- `bun/src/runtime/tsp/bin/tspserver.rs` — master entry, spawns
...
./tsp.sh build:host         # copy bun.exe -> dist/tspserver/
