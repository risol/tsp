# TSP v2 `.tsp` Module Format

> Phase 0 topic doc. Source of truth: `tsp-v2-specification.md` §3-§6
> and `tsp-v2-plan.md` §3-§6, §60 freeze items 1, 2, 3, 4.
...
`@fragment` decorators. Syntax is plain TSX. (See `tsp-v2-plan.md`
...
The rationale is in `tsp-v2-plan.md` §5.1: a `.tsp` is a generation
...
routing. The motivation is in `tsp-v2-plan.md` §4.2.1: HTTP method
...
surface in `ctx.params.<name>`. Optional catch-all is not in v2.0.
...
`tsp-v2-plan.md` §5.3 for the rationale (rebuild cost, dev-loop
stability).

## What is NOT in v2.0

These are explicitly deferred to v2.x and are NOT part of the
`.tsp` file contract in v2.0:
...
- Custom error pages (`routes/_404.tsp`, `routes/_500.tsp`). The
  host's built-in error pages are the v2.0 default.
- Middleware modules (`middleware.ts`). The v2.0 design is
