...
> Status: **Resolved in TSP v2**
> Discovered: 2026-08-28
> Resolved: 2026-08-29
> Affected: Linux x64 Bun builds embedding the TSP v2 runtime
> Severity: Blocker during TSP v2 startup or the first request
...
TSP v2 called `std::fs::canonicalize`. On Linux, Rust's implementation uses
...
embedded worker binary under the required `tspserver_v2` basename and sending
the first request:

```bash
cd /src
cp bun/build/release/bun-profile /tmp/tspserver_v2
cp -R tests/v2_smoke/routes /tmp/tsp-v2-routes

TSP_PORT=9215 \
TSP_ROUTES_DIR=/tmp/tsp-v2-routes \
...
`tspserver_v2` basename. This is important because the Bun multi-call binary
...
- `docs/v2/adr/0002-cross-allocator-ownership.md`.
...
- `scripts/smoke-tspserver-v2.sh` — embedded-worker regression test;
- `docs/v2/adr/0002-cross-allocator-ownership.md` — permanent design rule.
