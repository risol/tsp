 ...
 *
 * Provides cookie management functionality through dependency injection.
 * Uses WeakMap to store cookie operations for each request context.
 */
...
/**
 * WeakMap for storing cookie operations per request
 * Uses WeakMap so that when PageContext is garbage collected,
 * the associated cookie context will also be automatically cleaned up
 */
...
export function serializeCookie(
  ...
  // Encode name and value (special characters required)
...
/**
 * Create cookie manager for given request context
...
export function createCookieManager(ctx: PageContext): CookieManager {
  ...
  return {
    ...
    delete(
      ...
      // To delete cookie, set maxAge=0 and expires time to past
      // Must match the domain and path when set
...
/**
 * Extract Set-Cookie headers for given request context
 * Called by main.ts to add cookies to HTTP response
 * @param ctx Page context
 * @returns Set-Cookie header values array, or undefined if none
 */
