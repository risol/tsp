...
本文定义 TSP v2 的目标架构、运行时边界、`.tsp` 文件规范、Rust/JSC 交互模型、路由、请求上下文、响应模型、JSX runtime、依赖图、热更新、持久资源、构建发布和测试标准。

TSP v2 不以迁移 TSP v1 为主要约束，而以重新建立一个长期稳定、实现边界清晰、运行时状态可控的 Server Page Runtime 为目标。
...
TSP v2 不是 React Framework，也不是 Node/Bun application framework 的薄封装。
...
TSP v2 默认设计自己的 Server JSX runtime。

JSX 是语法；React 是其中一种 runtime。

TSP v2 不把 React Element 作为核心 ABI。

---

# 4. `.tsp` 文件规范

本章属于 **TSP v2 Language/Runtime Contract**，优先冻结。
...
不在 v2.0 首版支持 optional catch-all。
...
v2.0 可以暂不实现 component-level error boundary。
...
Fragment 是 TSP v2 的一等能力，但不成为另一种文件类型。
...
v2.0 不建议实现复杂 nested layout runtime。
...
未来如需 directory layout，可以在 v2.x 设计，不进入 v2.0 ABI。
...
v2 更推荐直接在 Bun Rust runtime 内实现 TSP host，或作为 fork 中隔离的 TSP native subsystem。
...
或者 route 优先于 static，需要在 v2 协议中明确。
...
v2.0 不应同时发明新的 npm package manager。
...
我建议 **v2.0 不实现通用 JS middleware chain**。
...
v2.0 不建议先做复杂 framework data cache。
...
## Phase 0 — 冻结 v2 Contract

产物：

```text
docs/v2/spec.md
docs/v2/tsp-module.md
docs/v2/jsx-runtime.md
docs/v2/context.md
```
...
当 v2 完成后，旧架构中的以下职责不应继续由 application TypeScript host 实现：
...
ADR-010 No generic middleware in v2.0
```
...
因为它决定整个 v2 hot reload runtime 是否成立。
...
如果 v2 最终需要用户先学几十个 framework primitives，说明 runtime 又变复杂了。
...
TSP v2 不应该是“把 `main.ts` 翻译成 Rust”。
