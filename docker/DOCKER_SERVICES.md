...
Samba AD 提供完整的 Active Directory 域控制器功能，完全兼容 Microsoft AD 的 LDAP 接口。

**连接信息：**

| 参数 | 值 |
|------|-----|
| 镜像 | docker.io/samba/samba-ad-dc:latest |
| 容器名 | tsp-samba-ad |
| 域名 | EXAMPLE.COM |
| NetBIOS 名 | EXAMPLE |
| 管理员 DN | CN=Administrator,CN=Users,DC=example,DC=com |
| 管理员密码 | P@ssw0rd123 |
| 管理工具 | http://localhost:8082 (LDAP Account Manager) |
...
| 53 | TCP+UDP | DNS |
| 88 | TCP+UDP | Kerberos |
| 135 | TCP | End Point Mapper (DCE-RPC) |
...
| DN | 密码 | 邮箱 |
|---|---|---|
| `CN=testuser1,CN=Users,DC=example,DC=com` | Test@123 | testuser1@example.com |
| `CN=testuser2,CN=Users,DC=example,DC=com` | Test@123 | testuser2@example.com |
| `CN=testuser3,CN=Users,DC=example,DC=com` | Test@123 | testuser3@example.com |
...
**注意事项：**
1. Samba AD 首次启动需要 5-10 分钟进行域控制器初始化
2. 域控制器会自动提供 DNS 服务
3. 完全兼容 Microsoft Active Directory 的 LDAP 接口
4. 支持标准 LDAP v3 协议
...
3. **查看容器日志**
   ...
   docker compose -f docker/docker-compose.yml logs samba-ad
   ```
...
└── docker/
    ...
    ├── docker-compose.server.yml   # TSP Server 专用 Compose
    ├── start.sh                    # 启动脚本
    ├── stop.sh                     # 停止脚本
    ├── restart.sh                  # 重启脚本
...
### TSP Application Test
```bash
# LDAP connection test
./tsp.sh test:unit

# E2E test (includes LDAP)
./tsp.sh test:e2e
```
...
- [Docker LDAP 配置](./config/samba/README.md)
- [TSP 文档](../README.md)
