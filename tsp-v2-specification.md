# TSP v2 Specification

**Status:** Draft 0.1  
**Target:** TSP v2.0  
**Compatibility:** Intentionally incompatible with TSP v1  
...
This document defines the normative contract of TSP v2.
...
This document intentionally does **not** describe the migration path from TSP v1 and does **not** require compatibility with v1 APIs or file formats.

Implementation plans, repository layout, milestones, and experimental alternatives belong in the separate TSP v2 Plan document.
...
A conforming TSP v2 runtime MUST implement every requirement marked MUST or MUST NOT.
...
TSP v2 is a server-side web application runtime with the following architectural model:
...
TSP v2 is **not** defined as a JavaScript web framework that happens to use a Rust executable.
...
The following invariants are fundamental TSP v2 rules.
...
React MUST NOT be required by the TSP v2 core runtime.

Applications MAY use React as a separate library if an adapter exists, but React semantics are not part of the TSP v2 language contract.
...
The v2.0 core `PageConfig` contract is:
...
A conforming v2.0 runtime MUST support explicit extensions.

A runtime MAY additionally support extension inference, but portable TSP v2 applications SHOULD NOT rely on it.
...
A relative dynamic import into the reloadable application graph MUST use a string-literal specifier in TSP v2.0.
...
Except for `/`, a trailing slash MUST NOT create a distinct route identity in v2.0.
...
TSP v2 defines a logical session service at `ctx.session`.
...
TSP v2 does not define React-style browser event binding or hydration.
...
React-style `ref` is not part of the TSP v2 core JSX contract.
...
React-style class components are not part of the TSP v2 core contract.
...
TSP v2.0 does not require component-level error boundaries.
...
The v2.0 default method is `GET`.
...
The following modules are reserved by TSP v2.
...
This isolation rule is deliberate and is part of the v2 architecture.
...
# 40. Rendering model in v2.0

The mandatory TSP v2.0 rendering contract is buffered server rendering.
...
Streaming HTML rendering is OPTIONAL in v2.0 and MUST use an explicit API or capability when introduced.
...
The canonical native TSP v2 configuration file is:
...
A conforming v2.0 implementation MUST NOT require `tsp.config.ts` merely to start the native HTTP runtime.
...
A canonical v2 page looks like:
...
The page demonstrates the intended v2 principles:
...
TSP v2 MUST support a deployment model where the native server executable is separable from application page source.
...
TSP v2.0 core does not define automatic page-output caching.
...
User-defined arbitrary middleware is **not** part of the mandatory TSP v2.0 page contract.
...
A special layout file convention is **not** part of the mandatory TSP v2.0 specification.
...
This avoids introducing hidden nested route lifecycles in v2.0.
...
Authentication policy is not encoded into the `.tsp` grammar in v2.0.
...
TSP v2 does not guarantee compatibility with:
...
A change requiring ordinary valid v2 `.tsp` source to be rewritten is normally a major-version change.
...
# 67. Frozen v2.0 protocol decisions

Before v2.0 is declared stable, the following decisions SHOULD be treated as protocol freeze candidates.
...
24. React/browser hydration is outside the core v2.0 contract.
...
At minimum, the suite MUST cover the following areas before v2.0 release.
...
# 69. Non-goals for TSP v2.0

The following are explicitly not required for v2.0 core conformance:
...
- compatibility with TSP v1 global APIs;
- preserving `main.ts`.
...
# 74. Minimal v2.0 implementation bar

A runtime MUST NOT be marketed as a conforming TSP v2.0 runtime unless it can demonstrate all of the following:
...
That sentence is the architectural compatibility target for TSP v2.
