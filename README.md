...
项目已包含 VS Code 配置文件，打开 `.tsp` 文件会自动识别。

### 配置说明

项目包含以下配置文件：
- `.vscode/settings.json` - 文件关联设置
- `.vscode/extensions.json` - 推荐扩展
- `.vscode/eta.code-snippets` - 代码片段

### 推荐安装的扩展

1. **EJS Language Support** - 提供 Eta/EJS 语法高亮
   - 搜索：`DigitalBrainstem EJS` 或 `EJS language support`

2. 安装后，打开任意 `.tsp` 文件，点击右下角语言模式，选择 **EJS**
...
| 缩写 | 说明 | 输出 |
|------|------|------|
| `eto` | Eta 输出 | `<%= expression %>` |
| `etou` | Eta 非转义输出 | `<%- expression %>` |
| `etc` | Eta 代码块 | `<% // code %>` |
| `etif` | Eta if 语句 | `<% if () { } %>` |
| `etfor` | Eta for 循环 | `<% for () { } %>` |
| `ethtml` | HTML5 模板 | 完整 HTML5 结构 |
