...
/// `"hash":"..."` value from the GET /bcrypt response so the
...
/// We need the status line separately because the zod test
/// asserts on `200` vs `400` to prove the validation path
/// works end-to-end.
...
// ---------------------------------------------------------------------------
// bcrypt runtime integration test (slice 17c)
//
// The TSP v2 wrap preamble inlines the pre-bundled bcryptjs
// 3.0.3 CJS via `include_str!` and exposes `__tspServer.bcrypt`
// so pages can call `hashSync` / `compareSync` / `genSaltSync`
// without an import step. We assert this end-to-end by
// spinning the real binary against a temp routes dir that
// contains `bcrypt.tsp`, sending the documented GET (hash
// generation) and POST (compare) requests, and checking the
// status + body shape. The `compare` flow is the strongest
// signal: 200 means the password matched the stored hash, 400
// means it didn't, and the page code branches on the actual
// bcrypt.verify() return value rather than swallowing it.
// ---------------------------------------------------------------------------

const BCRYPT_TSP: &str = r#"
// Generated by the bcrypt integration test in
// bun/src/runtime/tsp/tests/start_order.rs. Mirrors the
// production `routes/bcrypt.tsp` shape; the regression test
// inlines this rather than copying the file so the assertion
// is self-contained.
//
// Plan §16.4: framework API must be explicitly imported (or
// accessed via Context), NOT on globalThis. The page reaches
// bcrypt via `import { bcrypt } from "tsp:server"`; the host
// rewriter turns that into `const { bcrypt } = __tspServer;`
// against the frozen object the wrap preamble built.
...
#[test]
fn bcrypt_runtime_compiled_into_wrap_serves_hashed_passwords() {
    let Some(master) = locate_master() else {
        eprintln!(
            "skipping: tspserver_v2 binary not found under dist/tsp-v2/ \
             (run ./tsp.sh build:host first)"
    ...
    let routes_dir = temp_root.join("routes");
    std::fs::create_dir_all(&routes_dir).expect("routes dir should be creatable");
    std::fs::write(routes_dir.join("bcrypt.tsp"), BCRYPT_TSP).expect("bcrypt.tsp should be writable");
    ...
    // GET /bcrypt: produces { ok, rounds: 4, hash, salt }. The
    // `rounds: 4` and the `$2b$04$` hash prefix together prove
    // the actual bcrypt algorithm ran (not a stub).
    let (status_get, body_get) = http_get_status(port, "/bcrypt", Duration::from_secs(10));
    assert_eq!(
        status_get, 200,
        "GET /bcrypt must return 200, got {status_get} body={body_get:?}"
    );
    assert!(
        body_get.contains(r#""ok":true"#)
            && body_get.contains(r#""rounds":4"#)
            && body_get.contains(r#""hash":"$2b$04$"#)
            && body_get.contains(r#""salt":"$2b$04$"#),
        "GET /bcrypt body must contain the hash + rounds; got {body_get:?}"
    );

    // Parse the GET response so we can drive the POST with the
    // freshly-generated hash. Doing this end-to-end (not
    // hardcoding a known hash) is what catches a real
    // regression -- if the bcrypt code path were accidentally
    // stubbed or aliased to nanoid, the hash prefix would
    // change. The body is the raw response text; we do a
    // hand-rolled substring scan (no `serde_json` dependency
    // in the test crate) to extract the `"hash":"..."` value.
    let hash = extract_json_string_field(&body_get, "hash")
        .unwrap_or_else(|| panic!("GET /bcrypt body must carry a `hash` string field, got {body_get:?}"));

    // POST /bcrypt with the right password: 200 + matches: true.
    let (status_ok, body_ok) = http_post_status(
        port,
        "/bcrypt",
    ...
    assert_eq!(
        status_ok, 200,
        "POST /bcrypt with matching pw must return 200, got {status_ok} body={body_ok:?}"
    );
    assert!(
        body_ok.contains(r#""matches":true"#),
        "POST /bcrypt matching pw must report matches:true; got {body_ok:?}"
    );

    // POST /bcrypt with the wrong password: 400 + matches: false.
    // This is the actual authentication fail path -- the page
    // uses bcrypt.compareSync and branches on its return value.
    let (status_bad, body_bad) = http_post_status(
        port,
        "/bcrypt",
    ...
    assert_eq!(
        status_bad, 400,
        "POST /bcrypt with wrong pw must return 400, got {status_bad} body={body_bad:?}"
    );
    assert!(
        body_bad.contains(r#""matches":false"#),
        "POST /bcrypt wrong pw must report matches:false; got {body_bad:?}"
    );

    // POST /bcrypt with a non-JSON body: 400 + explicit error.
    let (status_garbage, body_garbage) =
        http_post_status(port, "/bcrypt", "not json", Duration::from_secs(5));
    assert_eq!(
        status_garbage, 400,
        "POST /bcrypt with non-JSON body must return 400, got {status_garbage} body={body_garbage:?}"
    );
    assert!(
        body_garbage.contains("body must be JSON"),
        "POST /bcrypt non-JSON body must carry the explicit error; got {body_garbage:?}"
    );
