...
- **文件操作**：上传、下载、删除、重命名、创建目录、批量移动
- **解压缩功能**：支持解压 ZIP、TAR、TAR.GZ 文件，压缩文件/目录为 ZIP
- **批量操作**：支持批量删除、批量移动、批量压缩
- **权限控制**：细粒度的操作权限配置
- **安全保护**：路径验证、文件类型限制、大小限制、ZIP 炸弹防护
...
1. 在文件列表中找到压缩文件（ZIP/TAR/TAR.GZ）
2. 点击文件名后的 📦 解压按钮
3. 在弹出的模态框中确认目标目录（默认为当前目录）
4. 点击"解压"按钮
5. 解压完成后自动刷新文件列表
...
2. 顶部工具栏显示"已选择 N 项"
3. 点击"📦 压缩选中项"按钮
4. 在弹出的模态框中输入 ZIP 文件名（例如：archive.zip）
5. 可选：勾选"包含父目录"（保留目录结构）
6. 点击"压缩"按钮
7. 压缩完成后自动刷新文件列表

### 安全限制

- **文件大小限制**：解压文件最大 1GB，压缩源文件总大小最大 500MB
- **文件数量限制**：单次解压最多 10,000 个文件（防止 ZIP 炸弹攻击）
- **格式限制**：只允许解压配置中指定的格式
- **路径验证**：所有路径都经过安全检查，防止路径穿越攻击

## 批量操作

### 批量选择

- 在文件列表中，每个文件/目录前都有一个复选框
- 勾选一个或多个文件后，顶部工具栏会显示"已选择 N 项"
- 工具栏按钮会根据选中数量自动启用或禁用
...
3. 在弹出的模态框中输入目标目录路径：
   - **绝对路径**：从根目录开始的完整路径
   - **相对路径**：相对于当前目录的路径（如 `../subdir`）
4. 点击"移动"按钮
5. 系统会逐个移动文件，显示移动结果（成功/失败统计）
6. 移动完成后自动刷新文件列表

**注意事项**：
- 目标目录必须存在，系统不会自动创建
- 如果目标目录中已有同名文件，该文件会跳过并计入失败统计
- 移动操作不可逆，请谨慎操作
- 默认已启用移动功能（`allowMove: true`），如需禁用请在配置中设置 `allowMove: false`
...
2. 点击顶部"🗑️ 删除选中项"按钮
3. 确认删除操作
4. 系统会逐个删除文件，显示删除结果（成功/失败统计）
5. 删除完成后自动刷新文件列表

**注意事项**：
- 删除操作不可逆，请谨慎操作
- 目录删除会递归删除其中的所有内容
- 需要配置中启用 `allowDelete: true` 才能使用此功能

### 批量压缩

详见"解压缩功能"章节的"压缩操作"部分。
